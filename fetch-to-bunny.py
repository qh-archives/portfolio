...
LIBRARY_ID = "642516"
CDN_HOST = "vz-53d1011b-a2d.b-cdn.net"

# All unique Cloudinary video URLs from the codebase (raw, no transforms)
cloudinary_videos = {
...
base = f"https://video.bunnycdn.com/library/{LIBRARY_ID}/videos/fetch"
results = []
errors = []

for i, (name, url) in enumerate(cloudinary_videos.items()):
    print(f"[{i+1}/{len(cloudinary_videos)}] Fetching {name}...", end=" ", flush=True)
    ...
    try:
        with urllib.request.urlopen(req) as resp:
            ...
            print(f"OK → {guid}")
            results.append((f"{name}.mp4", guid, name))
    except Exception as e:
        err_body = ""
        if hasattr(e, "read"):
            err_body = e.read().decode()
        print(f"ERROR: {e} {err_body}")
        errors.append((name, str(e), err_body))
    time.sleep(0.3)

# Write CSV
with open("bunny-upload-map.csv", "w") as f:
    f.write("filename,guid,title\n")
    for fn, guid, title in results:
        f.write(f"{fn},{guid},{title}\n")

print(f"\nDone! {len(results)} fetched, {len(errors)} errors.")
if errors:
    print("Errors:")
    for name, err, body in errors:
        print(f"  {name}: {err} {body}")
